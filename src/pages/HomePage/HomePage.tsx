...
const sliderDescriptions: Record<number, { ar: string; en: string }> = {
    ...
    3: { ar: "حوّل مناسباتك الخاصة إلى ذكريات خالدة في قاعاتنا المجهزة بالكامل.", en: "Turn your special occasions into timeless memories in our fully equipped halls." }, // Changed ID to 3 for EventHall
    2: { ar: "تذوق أشهى الأطباق العالمية والمحلية في مطاعم تلبي كل الأذواق.", en: "Savor the most delicious international and local dishes in restaurants that cater to all tastes." }, // Changed ID to 2 for Restaurant
...
const CategoryFeaturedSection: React.FC<{ category: Category; items: CardItem[]; loading: boolean; }> = ({ category, items, loading }) => {
  const { language } = useLanguage();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {(loading ? Array(3).fill(null) : items).map((it, idx) => (
          <ItemCard
            ...
            showStatus={false} // ✅ 4. تمرير الخاصية لإخفاء الحالة
          />
        ))}
      </div>
    </section>

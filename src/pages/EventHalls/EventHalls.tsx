...
const EventHallsPage = () => {
  ...
  const t = useCallback(
    (key: keyof typeof translations["en"], options?: { count: number }) => {
      const translation = translations[language][key];
      if (typeof translation === "function") return (translation as any)(options!);
      return translation;
    },
    [language]
  );
  ...
  const handleSearch = () => fetchEventHalls(searchQuery);
      ...
      <div className={`min-h-screen ${
        ...
        <div className="max-w-7xl mx-auto px-4 py-16">
          ...
          <div className="mb-8 text-center">
            ...
            >
              {loading ? t("loadingHalls") : t("foundHalls", { count: filteredHalls.length })}
            </h3>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
              {filteredHalls.map((hall) => (
                <div key={hall.id} className="animate-fade-in-up-item">
                  <ItemCard item={hall} loading={false} type="event_hall" rating={ratings[hall.id] || 0} />
                </div>
              ))}
            </div>
          ) : (
            <div className="text-center py-20 animate-fade-in-up-item">
              <FaMapSigns
                className={`text-7xl mx-auto mb-6 ${isDark ? "text-gray-600" : "text-gray-400"}`}
              />
              <h2 className={`text-3xl font-bold ${isDark ? "text-gray-400" : "text-gray-700"}`}>
                {t("noHallsTitle")}
              </h2>
